# Champ référentiel avancé

*Dernière mise à jour : 15 juillet 2025*
*Contact : via crisp ou tchap*
...
**Limite actuelle**
Le champ référentiel avancé fonctionne uniquement pour la recherche d’un identifiant unique (1:1). Il ne permet pas, pour le moment, de proposer une liste de résultats ou une autocomplétion lors de la saisie. L’usager doit donc connaître et saisir précisément l’identifiant recherché.
...
- [Présentation générale](#champ-référentiel-avancé)
- [1. Valider un identifiant (exemple: numéro RNB)](#1-valider-un-identifiant-exemple-numéro-rnb)
- [2. Pré-remplissage automatique des champs via le référentiel](#2-pré-remplissage-automatique-des-champs-via-le-référentiel)
- [3. Afficher des données publiques à l’usager](#3-afficher-des-données-publiques-à-lusager)
- [4. Afficher des données privées à l’instructeur](#4-afficher-des-données-privées-à-linstructeur)
- [Table de correspondance des types de données](#table-de-correspondance-des-types-de-données)
- [Que se passe-t-il en cas d’erreur lors de l’appel à l’API ?](#que-se-passe-t-il-en-cas-derreur-lors-de-lappel-à-lapi)
- [Questions fréquentes](#questions-fréquentes)
- [Évolutions à venir](#évolutions-à-venir)
- [Glossaire](#glossaire)
...
4.
    ...
    <figure><img src="../.gitbook/assets/Capture d’écran 2025-06-17 à 9.58.27 AM.png" alt=""><figcaption></figcaption></figure>
...
1. **Visualiser la réponse de l’API**
   Une fois l’URL et l’identifiant de test renseignés, la réponse JSON s’affiche dans l’interface.
   <figure><img src="../.gitbook/assets/Capture d’écran 2025-06-17 à 10.02.03 AM.png" alt=""></figure>
2. **Associer les propriétés du JSON aux champs du formulaire**
   Utilisez l’interface de mapping pour lier chaque propriété (ex : `addresses[0].street`, `status`) au champ cible du formulaire.
   <figure><img src="../.gitbook/assets/Capture d’écran 2025-06-17 à 10.03.52 AM.png" alt=""></figure>
3. **Gérer les répétitions (tableaux)**
   Si la donnée source est un tableau (ex : plusieurs adresses), chaque élément peut être mappé sur une répétition du formulaire.
   <figure><img src="../.gitbook/assets/Capture d’écran 2025-06-17 à 10.06.30 AM.png" alt=""></figure>

4. Un résumé du mapping s’affiche dans la fiche du champ référentiel : il liste les correspondances entre les données du référentiel et les champs du formulaire.

<figure><img src="../.gitbook/assets/Capture d’écran 2025-06-17 à 10.07.46 AM.png" alt=""><figcaption></figcaption></figure>
...
1. **Configurer le champ référentiel avancé**
   (voir les étapes précédentes pour l’URL et l’identifiant de test)
2. **Faire le mapping des propriétés à afficher**
...
<figure><img src="../.gitbook/assets/Capture d’écran 2025-06-17 à 10.07.46 AM.png" alt=""></figure>
...
1. **Configurer le champ référentiel avancé**
   (voir les étapes précédentes pour l’URL et l’identifiant de test)
2. **Faire le mapping des propriétés à afficher à l’instructeur**
   Dans l’interface de mapping, cochez l’option “Afficher à l’instructeur” pour chaque donnée que vous souhaitez rendre visible uniquement côté instructeur.
3. **Vérifier l’affichage**
   Lors d’un test, les données privées apparaissent dans la fiche du dossier, visibles uniquement par l’instructeur.

> **Astuce**
> Utilisez cette fonctionnalité pour transmettre des informations sensibles ou techniques qui ne concernent pas l’usager (ex : statut d’un bâtiment, données cadastrales, historique de contrôles).
...
**Q : Que faire si l'API change**
R : Si votre API change, il vous faudra mettre à jour le champ référentiel avancé à configurer

**Q : Quelles sont les évolutions à venir ?**
R : Elles seront nombreuses. Vous pouvez avoir un aperçu [ici](https://github.com/demarches-simplifiees/demarches-simplifiees.fr/issues/11161). Pour faire court :
* l'autocomplete
...
> Elle vous aide à savoir comment chaque type de donnée reçu depuis le référentiel (API) peut être relié à un champ du formulaire.
...
| integer                                  | integer_number                                 | Nombre entier                                    |
| float / number                           | decimal_number                                 | Nombre à virgule                                 |
| boolean                                  | yes_no, checkbox                               | Case à cocher ou Oui/Non                         |
...
| array de valeurs simples                 | multiple_drop_down_list                        | Liste à choix multiples                          |
...
| string                                   | drop_down_list (avec option “autre”)           | Saisie libre si activée                          |
...
> **Astuce**
> En cas d’erreur temporaire, l’usager n’a rien à faire : la plateforme gère les tentatives automatiquement. En cas d’erreur permanente, il doit vérifier l’identifiant saisi ou contacter l’assistance.
